...
def implied_common_shares(stock_code: str, name: str | None = None) -> Value:
    """KRX 시가총액 ÷ 최근 종가 = **상장 보통주식수**(내재).

    ⚠️ 이 값은 자기주식을 **포함**한다 — 자기주식도 상장 상태는 유지되므로 거래소
    시가총액에 들어간다. 그래서 DCF 주당가치의 분모(유통주식수 = 자기주식 제외)와는
    자기주식만큼 다르고, **그 차이는 오류가 아니라 정의 차이**다. 두 숫자가 한 답변에
    같이 나올 때는 이 사실을 밝혀야 한다(그러지 않아서 "분모가 미정" 이라는 지적을 받았다).
      실측(2026-08, 기아): 역산 390,412,998 vs 유통 388,602,725 = 자기주식 1,810,273주
