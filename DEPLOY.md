...
보이는 것: **사람별** 질문 수와 마지막 활동, 계정별 대화·질문·도구호출·실패 건수, 최근 질문
타임라인, 데이터 소스 사용 순위, 그리고 **대화 클릭 시 질문·답변·사용 소스 전문**.

### 한 계정을 여러 명이 쓸 때 (`APP_USERS=team:...`)

계정 하나를 팀이 공유하면 대화기록이 한 폴더에 섞여 질문의 주인을 알 수 없다. 그래서 로그인
직후 **이름을 한 번 묻는 팝업**이 뜨고, 그 이름이 쿠키에 담겨 이후 질문마다 찍힌다(관리자
화면의 "사람" 표). 사이드바의 이름을 누르면 바꿀 수 있고, 이미 저장된 질문의 작성자는 그대로
남는다.

**자기신고이므로 신원 증명이 아니다** — 아무 이름이나 넣을 수 있다. 권한은 여전히 계정
(`APP_USERS`·`ADMIN_USERS`)이 정하고, 대화기록 격리도 계정 단위다(같은 계정을 쓰는 사람들은
서로의 대화 목록을 본다). 사람별로 대화를 격리해야 한다면 계정을 사람별로 발급해야 한다.
