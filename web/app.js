...
/* 소스 실측 점검은 화면에서 뺐다(사용자 요청).
 * 서버의 GET /api/health/sources 는 남아 있어 필요할 때 직접 조회할 수 있고,
 * 실패는 서버 로그에 남는다(core/health.py) — 화면을 어지럽히지 않으면서
 * EDINET 때처럼 소스가 조용히 죽는 상황은 여전히 되짚을 수 있다. */
